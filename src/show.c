...
{
	fprintf(stderr, "Usage: %s %s { <interface> | all | interfaces } [public-key | private-key | listen-port | listen-dev | fwmark | peers | preshared-keys | endpoints | allowed-ips | latest-handshakes | transfer | persistent-keepalive | dump]\n", PROG_NAME, COMMAND_NAME);
}
...
static void pretty_print(struct wgdevice *device)
{
	...
	if (device->listenindex)
		terminal_printf("  " TERMINAL_BOLD "listenindex" TERMINAL_RESET ": 0x%x\n", device->listenindex);
...
static bool ugly_print(struct wgdevice *device, const char *param, bool with_interface)
{
	...
	if (!strcmp(param, "public-key")) {
		...
		if (device->fwmark)
			...
			printf("off\n");
	} else if (!strcmp(param, "listen-dev")) {
		if (with_interface)
			printf("%s\t", device->name);
		if (device->listenindex)
			printf("0x%x\n", device->listenindex);
